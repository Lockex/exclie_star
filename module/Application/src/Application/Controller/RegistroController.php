...
class RegistroController extends AbstractActionController
{
        ...
        if($this->getRequest()->isPost()) {
            ...
            if($form->isValid()) {
                ...
                if($identicalValidator->isValid($data->getAnswer())) {
                    ...
                    $message = $this->getTranslatorHelper()->translate('Su respuesta es incorrecta, proporcione la respuesta correcta.');
                }
            }
        }
    ...
    public function resetPasswordAction()
    {
        ...
        if($this->getRequest()->isPost()) {
            ...
            if($form->isValid()) {
                ...
                $entityManager = $this->getEntityManager();
                $user = $entityManager->createQuery("SELECT u FROM Application\Entity\Usuarios u WHERE u.email = '$usernameOrEmail' OR u.username = '$usernameOrEmail'")->getResult(\Doctrine\ORM\Query::HYDRATE_OBJECT);
                $user = $user[0];
                ...
                if(isset($user)) {
                    ...
                    $message = 'El usuario o correo electrónico están incorrectos.';
                }
            }
        }
    ...
    public function municipiosajaxAction()
    {
      $id = $this->getRequest()->getPost('estadoid');
      $query = $this->getEntityManager()->createQuery("SELECT m FROM CsnUser\Entity\Municipios m WHERE m.ESTADO = $id");
    ...
    {
      $usuario = $this->getRequest()->getPost('nombrestring');
      $query = $this->getEntityManager()->createQuery("SELECT u.username FROM CsnUser\Entity\User u WHERE u.username = '$usuario'");
    ...
    {
      $usuario = $this->getRequest()->getPost('emailstring');
      $query = $this->getEntityManager()->createQuery("SELECT u.email FROM CsnUser\Entity\User u WHERE u.email = '$usuario'");
      $usuarios = $query->getArrayResult();
      if($usuarios){
        $data = array('a' => '2');
        $resultado = new JsonModel(array($data));
